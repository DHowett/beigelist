...
- (id)enabledMerchants
{
	...
	NSMutableArray *returnArray = [NSMutableArray array];
	[returnArray addObjectsFromArray: merchants];
	[returnArray addObjectsFromArray: [[BLAppManager sharedAppManager] appliances]];
	return returnArray;
}
...
- (id)_loadApplianceWithInfo:(id)info
{
	NSArray *applianceMerchants = [[BLAppManager sharedAppManager] appliances];
	for (id merchant in applianceMerchants)
	{
		if ([merchant showInTopRow] && [[info key] hasSuffix:[merchant identifier]])
		{
			return [merchant applianceInstance];
		}
	}
...
- (id) topShelfAppliances
{
	...
	if ([appliances count] < 5)
	{
		for (id merchant in applianceMerchants)
		{
			if ([merchant showInTopRow] && ![merchant presentedInTopRow])
			{
				[merchant setPresentedInTopRow:YES];
				changed = YES;
			}
	...
	if (changed)
	{
		...
		Ivar ivar = class_getInstanceVariable(object_getClass(mmController), "_needsUpdateAppliances");
		BOOL* p_needsUpdate = (BOOL*)((char*) mmController + ivar_getOffset(ivar));
		*p_needsUpdate = YES;
		[mmController _updateAppliancesIfNeeded];
	}
